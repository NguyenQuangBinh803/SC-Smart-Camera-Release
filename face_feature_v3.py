...
class FaceAndMaskDetection:
    def __init__(self):
        self.sliding_size = 20
        self.window_width = self.sliding_size
        self.window_height = self.sliding_size
    ...
    def face_detection(self, frame, faceNet):
        (h, w) = frame.shape[:2]
    ...
    def mask_detection(self, faces, face_locations):
        if len(faces) > 0:
            try:
